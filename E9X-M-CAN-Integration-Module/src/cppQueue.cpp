...
bool __attribute__((nonnull)) cppQueue::push(const void * const record)
{
	...
	{
		if (impl == queue_FIFO)			{ inc_idx(&out, rec_nb, 0); }	// as oldest record is overwritten, increment out
...
bool cppQueue::drop(void)
{
	...
	if (impl == queue_FIFO)		{ inc_idx(&out, rec_nb, 0); }
	else if (impl == queue_LIFO)	{ dec_idx(&in, rec_nb, 0); }
	else					{ return false; }
