...
typedef enum enumcppQueueType {
	queue_FIFO = 0,	//!< First In First Out behavior
	queue_LIFO = 1	//!< Last In First Out behavior
} cppQueueType;
...
public:
	...
	**	\return nothing
	**/
	cppQueue(const size_t size_rec, const uint16_t nb_recs=20, const cppQueueType type=queue_FIFO, const bool overwrite=false, void * const pQDat=NULL, const size_t lenQDat=0);
